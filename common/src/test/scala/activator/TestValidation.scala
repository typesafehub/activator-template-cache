...
object TestValidation extends Specification {
  ...
  "A ProcessResult" should {
    ...
    "Support adding filename to error message" in {
      val file = new java.io.File("foo/foobar")
      val withFilename = err ensureErrorContainsFilename file
      withFilename must equalTo(ProcessFailure(Seq[ProcessError]("foobar: Failed!")))
      val secondTime = withFilename ensureErrorContainsFilename file
      secondTime must equalTo(withFilename)
      secondTime must beTheSameAs(withFilename)
    }
  }
