...
object PathFinder
{
	...
	val empty = new PathFinder { private[asbt] def addTo(fileSet: mutable.Set[File]) {} }
	def strict(files: Traversable[File]): PathFinder = apply(files)
	def apply(files: => Traversable[File]): PathFinder = new PathFinder {
		private[asbt] def addTo(fileSet: mutable.Set[File]) = fileSet ++= files
	}
...
sealed abstract class PathFinder
{
  ...
  /** Selects all descendant paths with a name that matches <code>include</code> and do not have an intermediate
	...
	* time of calling.  If the filesystem changes, two calls to this method might be different.*/
	...
	private[asbt] def addTo(fileSet: mutable.Set[File])
...
{
	private[asbt] def addTo(fileSet: mutable.Set[File]): Unit = if(asFile.exists) fileSet += asFile
}
...
{
	private[asbt] def addTo(fileSet: mutable.Set[File]): Unit =
		for(file <- parent.get)
			handleFile(file, fileSet)
}
...
{
	private[asbt] def addTo(pathSet: mutable.Set[File])
	{
